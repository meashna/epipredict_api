...
const symptomNameMapping = {
  ...
  'cyclical fever with chills and sweating': 'Cyclical_Fever_with_Chills_and_Sweating',
...
exports.createConsultation = async (req, res) => {
  ...
  try {
    ...
    const fastApiUrl = process.env.FASTAPI_URL || 'http://localhost:8000/predict'; // Use environment variable
    ...
    try {
      ...
      return res.status(502).json({ error: 'Failed to get prediction from prediction service' });
    }
    ...
    const activeSymptoms = Object.keys(populatedConsultation.symptoms.toObject()).filter(
      key => populatedConsultation.symptoms[key] === 1
    );
